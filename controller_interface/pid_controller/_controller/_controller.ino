 ...
 * Brief: Main entry point of the application. Contains setup() and loop().
 */
 ...
 void setup() {
     ...
     g_systemState.controlMode = CONTROL_MODE_SIGMOIDAL;
 
     // Initialize control modules
     // (We do an initial initSigmoidalController() in case we start ON)
     initSigmoidalController(g_systemState);
 ...
 void loop() {
     ...
     updateButtons();
     bool currentSystemOn = isSystemOn(); // local var
     ...
     if (!previousSystemOn && currentSystemOn) {
         ...
         Serial.println("[MAIN DEBUG] System turned ON (main loop) -> initSigmoidalController()");
         initSigmoidalController(g_systemState); // This resets the integrator, etc.
     } 
     else if (previousSystemOn && !currentSystemOn) {
         // System just turned OFF
         Serial.println("[MAIN DEBUG] System turned OFF (main loop) -> initSigmoidalController()");
         initSigmoidalController(g_systemState); 
         // Optionally call stopPump() here too:
         // stopPump();
     }
     ...
     if (wasModeTogglePressed()) {
         if (g_systemState.controlMode == CONTROL_MODE_SIGMOIDAL) {
             g_systemState.controlMode = CONTROL_MODE_CONST_VOLTAGE;
             Serial.println("[MAIN DEBUG] Mode changed -> CONSTANT VOLTAGE");
         } else {
             g_systemState.controlMode = CONTROL_MODE_SIGMOIDAL;
             Serial.println("[MAIN DEBUG] Mode changed -> SIGMOIDAL CONTROL");
         }
     }
 
     // 4) Acquire sensor inputs (store in g_systemState)
     ...
     if (g_systemState.systemOn) {
         // Only call a controller if the system is ON
         switch (g_systemState.controlMode) {
             case CONTROL_MODE_SIGMOIDAL:
                 Serial.println("[MAIN DEBUG] (ON) Calling updateSigmoidalController()...");
                 ...
                 Serial.println("[MAIN DEBUG] Returned from updateSigmoidalController()");
                 break;
         ...
         // Just ensure pump is off or put safe default outputs
         stopPump();
         desiredVoltage = 0.0f;
         pidFraction = 0.0f;
         pTerm = 0.0f;
     ...
     // 10) Delay loop for fluid response
