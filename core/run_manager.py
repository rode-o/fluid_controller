...
class RunManager:
    ...
    def __init__(self, data_root: str):
        """
        data_root: the base directory where runs are stored
        """
    ...
    def create_run_folders(self, test_name: str, user_name: str, timestamp: str):
        ...
        """
        self.run_folder = os.path.join(self.data_root, f"test_{test_name}_{user_name}_{timestamp}")
    ...
    def open_csv(self, test_name: str, user_name: str, timestamp: str):
        """
        Create and open the CSV file in raw_data_dir, write header.
        """
        ...
        header = ["timeMs", "flow", "setpt", "temp", "bubble", "volt", "on"]
        self.csv_writer.writerow(header)

    def write_csv_row(self, row):
        """
        Append a row (list/tuple) to the CSV.
        """
        if self.csv_writer:
    ...
    def run_post_analysis(self, stable_window, stable_threshold, fluid_density=1.0):
        """
        After capture stops, call analyze_data with the relevant subfolders,
        including fluid_density for mass calculation.
        """
        # Locate the CSV we just wrote
        raw_csv = None
